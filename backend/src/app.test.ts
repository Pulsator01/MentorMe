...
describe('MentorMe backend workflow', () => {
  ...
  it('returns the authenticated user context via /me', async () => {
    const { app } = await buildTestApp()

    const meRes = await app.inject({
      method: 'GET',
      url: '/me',
      headers: { [TEST_AUTH_HEADER]: 'aarav.sharma@mentorme.test' },
    })
  ...
  it('scopes venture data to the authenticated founder and creates a new request in cfe_review', async () => {
    ...
    const venturesRes = await app.inject({
      method: 'GET',
      url: '/ventures',
      headers: { [TEST_AUTH_HEADER]: founderToken },
    })
  ...
  it('links a signed-in mentor to a same-name profile when the profile email was generated by the roster form', async () => {
    const { app, repository } = await buildTestApp(async (state) => {
      state.users.push({
        id: 'user-mentor-kavya',
        organizationId: state.organization.id,
        cohortId: state.cohort.id,
        email: 'kavya.real@example.test',
        name: 'Kavya Rao',
        role: 'mentor',
      })
      state.mentors.push({
        ...
        organizationId: state.organization.id,
        name: 'Kavya Rao',
        email: 'kavya.rao@mentorme.test',
        title: 'Clinical pilot mentor',
        location: 'Remote',
        focus: ['Clinical pilots'],
        stages: ['Pilot'],
        domains: ['Healthtech'],
        tolerance: 'Medium',
        monthlyLimit: 3,
        ...
        responseWindow: '72h',
        calendlyUrl: '',
        bio: 'Helps teams prepare for clinical pilots.',
      })
      state.requests.push({
        id: 'REQ-902',
        organizationId: state.organization.id,
        ...
        mentorId: 'm-kavya-rao',
        stage: 'Pilot',
        trl: 5,
        brl: 4,
        status: 'awaiting_mentor',
        challenge: 'Need clinical pilot preparation support.',
        desiredOutcome: 'Confirm a clear pilot readiness checklist.',
        mentorNotes: '',
    ...
    const mentorToken = await loginAs(app, 'kavya.real@example.test')
    ...
    const listBody = parseJson<{
      mentor: { id: string; email: string }
      actions: Array<{ request: { id: string } }>
    }>(listRes)
    expect(listBody.mentor).toMatchObject({
      id: 'm-kavya-rao',
      email: 'kavya.real@example.test',
    })
    expect(listBody.actions.map((action) => action.request.id)).toContain('REQ-902')
  ...
  it('generates a structured meeting summary through the AI endpoint', async () => {
    ...
    const response = await app.inject({
      ...
      headers: { [TEST_AUTH_HEADER]: studentToken },
      payload: {
        ventureName: 'EcoDrone Systems',
  ...
  it('rejects unauthenticated notification stream requests', async () => {
    ...
    expect(notificationsRes.statusCode).toBeGreaterThanOrEqual(400)
    expect(notificationsRes.statusCode).toBeLessThan(500)
  })
  ...
  it('reports the seeded founder as having venture access (no further wizard needed)', async () => {
    ...
    const statusRes = await app.inject({
      method: 'GET',
      url: '/me/onboarding',
      headers: { [TEST_AUTH_HEADER]: token },
    })
  ...
  it('walks a fresh founder through the onboarding wizard and creates a venture', async () => {
    ...
    const beforeRes = await app.inject({
      method: 'GET',
      url: '/me/onboarding',
      headers: { [TEST_AUTH_HEADER]: 'maya.greenfield@mentorme.test' },
    })
  ...
  it('rejects founder onboarding when required fields are missing', async () => {
    ...
    const wizardRes = await app.inject({
      method: 'POST',
      url: '/onboarding/founder',
      headers: { [TEST_AUTH_HEADER]: 'patchy.founder@mentorme.test' },
      payload: {
        ventureName: 'Tiny',
        domain: 'X',
      },
  ...
  it('lets an invited user accept after registering with the same email and role', async () => {
    ...
    const acceptRes = await app.inject({
      ...
      headers: { [TEST_AUTH_HEADER]: 'accept.test@mentorme.test' },
    })
...
function loginAs(_app: Awaited<ReturnType<typeof createApp>>, email: string) {
  return email
}
